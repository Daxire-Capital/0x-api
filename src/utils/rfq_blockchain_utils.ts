...
export class RfqBlockchainUtils {
     ...
     */
    public async transformTxDataToTransactionRequestAsync(
        txOptions: Partial<TxData>,
        chainId?: ChainId,
        callData?: utils.BytesLike,
    ): Promise<providers.TransactionRequest> {
        let from = txOptions.from;
        if (!from) {
            if (!this._ethersWallet) {
                throw new Error("An Ethers wallet must be provided if no 'from' address is specified");
            }
            from = await this._ethersWallet.getAddress();
        }
